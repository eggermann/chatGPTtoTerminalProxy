...
PROMPT_CONCATOR_ENABLED="false"
PROMPT_CONCATOR_PROMPT="Project context generated by prompt-concator. Use these files as the current source of truth."
PROMPT_CONCATOR_TEXT_EDIT="false"
PROMPT_CONCATOR_IGNORE_GITIGNORE="false"
PROMPT_CONCATOR_PID_FILE="$INBOX_DIR/prompt-concator.pid"
PROMPT_CONCATOR_LOG_FILE="$INBOX_DIR/prompt-concator.log"
...
if command -v python3 >/dev/null 2>&1 && [ -f "$CONFIG_FILE" ]; then
  while IFS='=' read -r key value; do
    case "$key" in
      ...
      promptConcatorEnabled) PROMPT_CONCATOR_ENABLED="$value" ;;
      promptConcatorPrompt) PROMPT_CONCATOR_PROMPT="$value" ;;
      promptConcatorTextEdit) PROMPT_CONCATOR_TEXT_EDIT="$value" ;;
      promptConcatorIgnoreGitignore) PROMPT_CONCATOR_IGNORE_GITIGNORE="$value" ;;
    esac
  done < <(
...
prompt_concator = data.get("promptConcator", {})
if isinstance(prompt_concator, dict):
    for key, env_key in (
        ("enabled", "promptConcatorEnabled"),
        ("prompt", "promptConcatorPrompt"),
        ("textEdit", "promptConcatorTextEdit"),
        ("ignoreGitignore", "promptConcatorIgnoreGitignore"),
    ):
        if key in prompt_concator:
            value = prompt_concator[key]
            if isinstance(value, bool):
                value = str(value).lower()
            print(f"{env_key}={value}")
PY
...
prompt_concator_pid() {
  [ -f "$PROMPT_CONCATOR_PID_FILE" ] || return 1
  cat "$PROMPT_CONCATOR_PID_FILE" 2>/dev/null || return 1
}

prompt_concator_is_running() {
  local pid

  pid="$(prompt_concator_pid)" || return 1
  case "$pid" in
    ''|*[!0-9]*)
      return 1
      ;;
...
stop_prompt_concator() {
  local pid

  [ -f "$PROMPT_CONCATOR_PID_FILE" ] || return 0
  pid="$(cat "$PROMPT_CONCATOR_PID_FILE" 2>/dev/null || true)"
  case "$pid" in
    ''|*[!0-9]*)
      rm -f "$PROMPT_CONCATOR_PID_FILE"
      return 0
      ;;
  esac
...
start_prompt_concator() {
  local -a cmd

  if [ "$PROMPT_CONCATOR_ENABLED" != "true" ]; then
    stop_prompt_concator
    return 0
  fi

  if ! command -v npx >/dev/null 2>&1; then
    echo "prompt-concator skipped: npx not found." | tee -a "$LOG_FILE"
    return 0
  fi

  if prompt_concator_is_running; then
    echo "prompt-concator already running: $(prompt_concator_pid)"
    return 0
  fi

  cmd=(npx --yes prompt-concator -prompt "$PROMPT_CONCATOR_PROMPT" --watch --excludeFolders .codex-inbox)
  if [ "$PROMPT_CONCATOR_TEXT_EDIT" = "true" ]; then
    cmd+=(--textEdit)
  fi
  if [ "$PROMPT_CONCATOR_IGNORE_GITIGNORE" = "true" ]; then
    cmd+=(--ignore-gitignore)
  fi

  (
    cd "$PROJECT_DIR" || exit 1
    nohup "${cmd[@]}" >>"$PROMPT_CONCATOR_LOG_FILE" 2>&1 &
    echo $! > "$PROMPT_CONCATOR_PID_FILE"
  )

  echo "prompt-concator watcher started: $PROMPT_CONCATOR_PID_FILE"
}
...
echo "Prompt Concator enabled: $PROMPT_CONCATOR_ENABLED"
echo ""
